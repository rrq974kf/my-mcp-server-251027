...
[![smithery badge](https://smithery.ai/badge/@rrq974kf/my-mcp-server-251027)](https://smithery.ai/server/@rrq974kf/my-mcp-server-251027)
...
To install TypeScript MCP Server 보일러플레이트 automatically via [Smithery](https://smithery.ai/server/@rrq974kf/my-mcp-server-251027):

```bash
npx -y @smithery/cli install @rrq974kf/my-mcp-server-251027
```
...
이 서버에는 AI를 사용한 텍스트-이미지 생성 도구가 포함되어 있습니다.
...
프로젝트 루트에 `.env` 파일을 생성하고 토큰을 추가하세요:
...
> 💡 **Hugging Face 토큰 발급하기**: [Hugging Face 설정](https://huggingface.co/settings/tokens)에서 새 토큰을 생성할 수 있습니다.
> 
...
```typescript
// Cursor MCP에서 사용:
// "우주비행사가 말을 타고 있는 이미지를 생성해줘"
// "beautiful sunset over mountains"
```

이미지는 base64 인코딩된 PNG 형식으로 반환됩니다.

## 📦 주요 의존성

-   **@modelcontextprotocol/sdk**: MCP 프로토콜 구현을 위한 공식 SDK
-   **@huggingface/inference**: Hugging Face AI 모델 추론을 위한 클라이언트
